...
```text
.repobud/skills/<skill-id>/SKILL.md
```
...
1. A repository `On` or `Off` entry in `.repobud/config.json`.
2. The global `On` or `Off` default in `repobud.json`.
...
RepoBud prefers a directory symlink so canonical content is not duplicated.
...
standard root directly. RepoBud does not create a redundant `.cursor` copy for
...
```text
<skill-id>/.repobud/overlays/claude-code.yaml
<skill-id>/.repobud/overlays/cursor.yaml
```
...
`src/vs/workbench/contrib/repoBud/test/fixtures/skillProjectionClients.json`. Current
