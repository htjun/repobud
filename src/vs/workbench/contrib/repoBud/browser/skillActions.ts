...
export const MANAGE_GLOBAL_SKILL_LIBRARY_COMMAND_ID = 'repoBud.manageGlobalSkillLibrary';
export const REFRESH_CONTEXT_SKILLS_COMMAND_ID = 'repoBud.refreshSkills';
...
registerAction2(class RefreshContextSkillsAction extends Action2 {
	constructor() {
		super({
			...
			title: localize2('repoBudRefreshSkills', 'Refresh Skills'),
...
registerAction2(class ManageGlobalSkillLibraryAction extends Action2 {
	constructor() {
		super({
			...
			title: localize2('repoBudManageGlobalSkillLibrary', 'Manage Global Skill Library...'),
			f1: false,
			menu: {
				id: MenuId.ViewContainerTitle,
	...
	override async run(accessor: ServicesAccessor): Promise<void> {
		...
		if (skillService.snapshot.globalSkills.length === 0) {
			notificationService.info(localize(
				'repoBudGlobalSkillLibraryEmpty',
		...
		const pickedSkill = await quickInputService.pick<IGlobalSkillQuickPickItem>(
			skillService.snapshot.globalSkills.map(skill => ({
				...
				description: skill.activation === 'on'
					? localize('repoBudGlobalSkillEnabled', 'Enabled globally')
					: localize('repoBudGlobalSkillDisabled', 'Disabled globally'),
				detail: skill.issue ?? skill.description,
				skill,
			})),
			{
				title: localize('repoBudGlobalSkillLibraryTitle', 'Global Skill Library'),
				placeHolder: localize(
					'repoBudGlobalSkillLibraryPlaceholder',
		...
		const pickedActivation = await quickInputService.pick<IGlobalActivationQuickPickItem>([{
			label: `$(check) ${localize('repoBudEnableSkillGlobally', 'On')}`,
			description: localize('repoBudEnableSkillGloballyDescription', 'Enable by default in repositories.'),
			activation: 'on',
		}, {
			label: `$(circle-slash) ${localize('repoBudDisableSkillGlobally', 'Off')}`,
			description: localize('repoBudDisableSkillGloballyDescription', 'Keep available but disabled by default.'),
			activation: 'off',
		}], {
			title: pickedSkill.skill.name,
			placeHolder: localize('repoBudGlobalSkillActivationPlaceholder', 'Choose the global default'),
		});
		if (!pickedActivation) {
			return;
		}
