...
const projectionManifestStorageKey = 'repoBud.skillProjection.manifests';
...
export class ContextSkillService extends Disposable implements IContextSkillService {
	...
	constructor(
		...
		@IRepoBudSkillProjectionService private readonly projectionService: IRepoBudSkillProjectionService,
	...
	private async resolveClientCompatibility(
		...
		return Promise.all(projectionClients.map(async client => {
			const overlay = joinPath(directory, '.repobud', 'overlays', `${client}.yaml`);
