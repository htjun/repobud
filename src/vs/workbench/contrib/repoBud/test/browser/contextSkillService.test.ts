...
suite('ContextSkillService', () => {
	...
	test('resolves active-repository Skills and persists Inherit, On, and Off overrides', async () => {
		await Promise.all([
			...
			writeSkill(joinPath(activeRepository, '.repobud', 'skills'), 'release', 'Release', 'Prepare releases.'),
		]);
	...
	test('reports conflicting and invalid Skill definitions', async () => {
		await Promise.all([
			...
			writeSkill(joinPath(activeRepository, '.repobud', 'skills'), 'review', 'Repository Review', 'Review locally.'),
			writeSkill(joinPath(activeRepository, '.repobud', 'skills'), 'invalid', 'Invalid'),
		]);
		skillService = disposables.add(new ContextSkillService(
			fileService,
			catalogService,
	...
	test('resolves client targets and keeps managed-copy manifests in machine storage', async () => {
		await Promise.all([
			writeSkill(joinPath(globalRepository, 'skills'), 'review', 'Review', 'Review changes.'),
			writeSkill(
				joinPath(activeRepository, '.repobud', 'skills'),
				'release',
				'Release',
				'Prepare releases.'
			),
		...
		const stored = storageService.getObject(
			'repoBud.skillProjection.manifests',
	...
	test('validates Agent Skills compatibility and scopes overlays to one client', async () => {
		...
		await fileService.createFolder(joinPath(skillDirectory, '.repobud', 'overlays'));
		...
		await fileService.writeFile(
			joinPath(skillDirectory, '.repobud', 'overlays', 'cursor.yaml'),
		...
		assert.strictEqual(
			...
			'/project/.repobud/skills/release/.repobud/overlays/cursor.yaml'
		);
