...
} from '../../../../../platform/repoBud/common/githubCredentialService.js';
...
suite('ContextConnectionService', () => {
	...
	test('supports multiple accounts with independent global and repository selection', async () => {
		...
		const portableFiles = [
			await fileService.readFile(joinPath(activeRepository, '.repobud', 'config.json')),
			await fileService.readFile(joinPath(globalRepository, 'repobud.json')),
