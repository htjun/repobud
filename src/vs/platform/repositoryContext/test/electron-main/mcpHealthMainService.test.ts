...
import { RepositoryContextMcpHealthMainService } from '../../electron-main/mcpHealthMainService.js';

suite('RepositoryContextMcpHealthMainService', () => {
	ensureNoDisposablesAreLeakedInTestSuite();

	let fixtureRoot: string;
	let service: RepositoryContextMcpHealthMainService;

	setup(async () => {
		fixtureRoot = await fs.mkdtemp(join(tmpdir(), 'repository-context-mcp-health-'));
		service = new RepositoryContextMcpHealthMainService();
	});
	...
	test('initializes a stdio server with a minimal environment', async () => {
		...
		await fs.writeFile(server, [
			...
			'  const capabilities = process.env.REPOSITORY_CONTEXT_TEST_SECRET ? { leaked: {} } : { tools: {}, resources: {} };',
			'  process.stdout.write(JSON.stringify({ jsonrpc: "2.0", id: request.id, result: { protocolVersion: "2025-06-18", capabilities } }) + "\\n");',
			'});',
		].join('\n'));
		process.env.REPOSITORY_CONTEXT_TEST_SECRET = 'must-not-leak';
		try {
			...
			delete process.env.REPOSITORY_CONTEXT_TEST_SECRET;
		}
	});
