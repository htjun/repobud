...
export class RepositoryContextPluginPackageMainService implements IRepositoryContextPluginPackageService {
	...
	private async materialize(source: IPluginSourceRequest): Promise<IMaterializedPlugin> {
		...
		const root = await fs.mkdtemp(join(tmpdir(), 'repository-context-plugin-'));
		try {
