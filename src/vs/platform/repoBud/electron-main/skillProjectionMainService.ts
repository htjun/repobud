...
export class RepoBudSkillProjectionMainService implements IRepoBudSkillProjectionService {
	...
	async importChanges(request: ISkillProjectionRequest): Promise<ISkillProjectionResult> {
		...
		const temporarySource = `${paths.source}.repobud-import-${randomUUID()}`;
		const backupSource = `${paths.source}.repobud-backup-${randomUUID()}`;
	...
	private async validateRequest(request: ISkillProjectionRequest): Promise<IValidatedProjectionPaths> {
		...
		if (request.overlay) {
			...
			const overlayRoot = join(source, '.repobud', 'overlays');
			if (!this.isEqualOrParent(overlayRoot, overlay) || basename(overlay) !== `${request.client}.yaml`) {
				throw new Error('A client overlay must use the canonical .repobud/overlays directory.');
			}
	...
	private async copyProjectionInput(paths: IValidatedProjectionPaths): Promise<void> {
		const overlayDirectory = join(paths.source, '.repobud', 'overlays');
	...
	private async hashProjectionInput(paths: IValidatedProjectionPaths): Promise<string> {
		const sharedHash = await this.hashDirectory(
			paths.source,
			relativePath => !this.isEqualOrParent('.repobud/overlays', relativePath)
		);
		if (!paths.overlay) {
			return sharedHash;
		}
