...
import { RepoBudPluginPackageMainService } from '../../electron-main/pluginPackageMainService.js';

const execFileAsync = promisify(execFile);

suite('RepoBudPluginPackageMainService', () => {
	ensureNoDisposablesAreLeakedInTestSuite();

	let fixtureRoot: string;
	let configurationRepository: string;
	let source: string;
	let service: RepoBudPluginPackageMainService;

	setup(async () => {
		fixtureRoot = await fs.mkdtemp(join(tmpdir(), 'repobud-plugin-test-'));
		...
		service = new RepoBudPluginPackageMainService({
