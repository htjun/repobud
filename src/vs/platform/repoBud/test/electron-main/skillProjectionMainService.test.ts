...
import { RepoBudSkillProjectionMainService } from '../../electron-main/skillProjectionMainService.js';

suite('RepoBudSkillProjectionMainService', () => {
	ensureNoDisposablesAreLeakedInTestSuite();

	let fixtureRoot: string;
	let service: RepoBudSkillProjectionMainService;

	setup(async () => {
		fixtureRoot = await fs.mkdtemp(join(tmpdir(), 'repobud-projection-'));
		service = new RepoBudSkillProjectionMainService();
	});
	...
	test('materializes a client overlay only in its target', async () => {
		...
		const overlayDirectory = join(baseRequest.source.fsPath, '.repobud', 'overlays');
		...
		assert.strictEqual(
			await fs.stat(join(claudeRequest.target.fsPath, '.repobud', 'overlays'))
				.then(() => true, () => false),
			false
		);
		await assert.rejects(
