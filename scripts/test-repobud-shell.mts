...
import {
	...
	redactSmokeText,
} from './repobud-smoke-artifacts.mts';

const repositoryRoot = process.cwd();
const applicationPath = process.env.REPOBUD_APP_PATH ??
	join(repositoryRoot, '..', 'VSCode-darwin-arm64', 'RepoBud.app');
const executablePath = join(applicationPath, 'Contents', 'MacOS', 'RepoBud');
...
async function writePluginFixture(root, version, readme) {
	...
	await writeFile(join(root, 'repobud-plugin.json'), `${JSON.stringify({
		schemaVersion: 1,
		id: 'review-tools',
		name: 'Review Tools',
		version,
...
async function main() {
	...
	const temporaryBase = process.env.REPOBUD_SMOKE_TMPDIR ?? '/tmp';
	const temporaryRoot = await mkdtemp(join(temporaryBase, 'repobud-smoke-'));
	...
	try {
		...
		await Promise.all([
			...
			writeFile(
				...
				JSON.stringify([{
					...
					key: 'cmd+alt+c',
					command: 'repoBud.manageConfigurationRepository',
					args: {
						action: 'initialize',
						...
						key: 'cmd+alt+p',
						command: 'repoBud.managePluginLibrary',
					}, {
						key: 'cmd+alt+u',
						command: 'repoBud.checkPluginUpdates',
					}])
				),
		]);
		...
		runGit(pluginSourcePath, ['config', 'user.name', 'RepoBud Smoke']);
		...
		runGit(fixturePath, ['config', 'user.name', 'RepoBud Smoke']);
		runGit(fixturePath, ['config', 'user.email', 'smoke@example.invalid']);
		await mkdir(join(fixturePath, '.repobud'), { recursive: true });
		await Promise.all([
			...
			writeSkillFixture(join(fixturePath, '.repobud', 'skills'), 'release', 'Release', 'Prepare repository releases.'),
			writeSkillFixture(join(fixturePath, '.repobud', 'skills'), 'conflict', 'Repository Conflict', 'Repository conflicting definition.'),
			...
			writeIntegrationFixture(join(fixturePath, '.repobud', 'integrations'), {
				id: 'remote-docs',
				name: 'Remote docs',
			...
			writeFile(join(fixturePath, '.repobud', 'config.json'), [
				'{',
				'\t"version": 1,',
				'\t"scope": "repository",',
				'\t"skills": {',
		...
		child = spawn(executablePath, [
			...
			env: createIsolatedApplicationEnvironment({
				...
				REPOBUD_GITHUB_USER_URL: githubFixture.url,
				REPOBUD_KEYCHAIN_NAMESPACE: keychainNamespace,
			}),
		...
		const initializedConfigurationFile = join(initializedConfigurationPath, 'repobud.json');
		await waitFor(
		...
		assert.equal(runGit(initializedConfigurationPath, ['status', '--porcelain']), '?? repobud.json');
		assert.notEqual(
		...
		assert.deepEqual(
			(await readdir(initializedConfigurationPath)).filter(name => name.includes('repobud-tmp')),
			[],
			'Atomic write temporary files were left behind.'
		);

		await page.keyboard.press('Meta+Alt+E');
		const existingConfigurationFile = join(existingConfigurationPath, 'repobud.json');
		await waitFor(
		...
		assert.deepEqual(
			...
			['?? integrations/', '?? repobud.json', '?? skills/']
		);
		assert.notEqual(
		...
		await page.locator('.repobud-skills-context-name').getByText('fixture', { exact: true }).waitFor();
		...
		const reviewSkill = page.locator('.repobud-skill-row[data-skill-id="review"]');
		const releaseSkill = page.locator('.repobud-skill-row[data-skill-id="release"]');
		const conflictSkill = page.locator('.repobud-skill-row[data-skill-id="conflict"]');
		...
		await waitFor(async () => {
			const configuration = JSON.parse(await readFile(
				join(fixturePath, '.repobud', 'config.json'),
				'utf8'
			));
			return configuration.skills.release?.activation === 'on';
		}, 'Repository Skill On override was not persisted.');
		const codexProjection = releaseSkill.locator('.repobud-skill-client-row').filter({ hasText: 'Codex' });
		const claudeProjection = releaseSkill.locator('.repobud-skill-client-row').filter({ hasText: 'Claude Code' });
		const cursorProjection = releaseSkill.locator('.repobud-skill-client-row').filter({ hasText: 'Cursor' });
		...
		const canonicalReleasePath = join(fixturePath, '.repobud', 'skills', 'release');
		await waitFor(
		...
		assert.doesNotMatch(
			await readFile(join(fixturePath, '.repobud', 'config.json'), 'utf8'),
		...
		const localIntegration = page.locator(
			'.repobud-integration-row[data-integration-id="local-tools"]'
		);
		const remoteIntegration = page.locator(
			'.repobud-integration-row[data-integration-id="remote-docs"]'
		);
		...
		const alphaConnection = remoteIntegration
			.locator('.repobud-integration-connection')
		...
		assert.ok(githubFixture.requests.every(
			request => request.userAgent?.startsWith('RepoBud/')
		));
		...
		const pluginSkill = page.locator(
			'.repobud-skill-row[data-skill-id="plugin-review"]'
		);
		...
		const pluginIntegration = page.locator(
			'.repobud-integration-row[data-integration-id="plugin-issues"]'
		);
		...
		assert.equal(
			JSON.parse(await readFile(
				join(installedPluginPath, 'repobud-plugin.json'),
				'utf8'
			)).version,
			'1.0.0',
		...
		await page.locator('.repobud-integration-row[data-integration-id="remote-docs"]')
		...
		console.log('RepoBud packaged shell smoke test passed.');
	} catch (error) {
		...
		if (artifactDirectory) {
			console.error(`Sanitized RepoBud smoke artifacts: ${artifactDirectory}`);
		}
